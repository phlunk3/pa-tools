...
class Afford
{
	...
	public function execute()
	{
		...
		if(!$planet) 
			if (!$this->tg)
				return response()->json(['error' => sprintf('No planet with coords %s:%s:%s found.', $this->x, $this->y, $this->z)], 422);
			else
				return sprintf('No planet with coords %s:%s:%s found.', $this->x, $this->y, $this->z);
		
		$ship = Ship::where('name', 'LIKE', '%' . $this->ship . '%')->first();

		if(empty($planet->latestP)) 
			if (!$this->tg)
				return response()->json(['error' => sprintf('No planet scan available on %d:%d:%d.', $this->x, $this->y, $this->z)], 422);
		else
			return sprintf('No planet scan available on %d:%d:%d.', $this->x, $this->y, $this->z);
