...
class MakeBattleCalc
{
	...
	public function execute()
	{
		...
		if (isset($this->teamups)):
			
			foreach ($this->teamups as $teamup):
				$attNum++;
				$this->addPlanetFromTeamup($teamup['x'], $teamup['y'], $teamup['z'], $teamup['type'], $attNum, 9, $teamup['fleets']);
			endforeach;
	...
	public function addPlanetFromTeamup($x, $y, $z, $type, $num, $id, $fleets)
	{
		$planet = Planet::with([
			'latestP',
			'latestP.scan',
		...
		foreach ($fleets as $fleet):
			$shipId =  $fleet['ship_id'] - 1;
			$this->params[$type . '_' . $num . '_' . $shipId] = $fleet['amount'];
		endforeach;
