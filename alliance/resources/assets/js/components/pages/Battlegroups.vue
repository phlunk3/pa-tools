			...
			<div class="col-md-12">
				...
				<div class="card card-default mb-3 has-table" v-if="battlegroups">
					<div class="card-header">
						Battlegroups<br/>
						<small>Scores are accumulated by planets while they are in the BG only. Scores are shown by average so smaller Battlegroups are not at a disadvantage.</small>
					</div>
						...
						<table id="alliances" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
							...
							<tbody>
								<tr v-for="(battlegroup, index) in battlegroups" v-bind:class="{ is_alliance: battlegroup.is_bg }">
									...
									<td style="width: 100%">{{ battlegroup.name }}<span v-if="settings.role == 'Admin' || settings.role == 'BC' || battlegroup.matches == true"><router-link :to="{ name: 'battlegroup', params: { id: battlegroup.id }}">&nbsp;[view]</router-link></span></td>
		...
		methods: {
			loadBattlegroups: function() {
				...
				.then((response) => {
					...
					for (this.i=0;this.i < this.battlegroups.length;this.i++) {
						for (this.a=0;this.a < this.battlegroups[this.i].members.length;this.a++) {
							if (this.battlegroups[this.i].members[this.a].id == this.settings.user.id) {
								console.log('User found in ' + this.battlegroups[this.i].id);
								this.battlegroups[this.i].matches = true;
							}
						}
					}
					this.loading = false;
			...
			handleSubmit() {
				if (this.name.length > 9) {
					alert('Battlegroup name too long, maximum 9 characters');
					return;
				} else {
