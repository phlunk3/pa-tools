					...
					<div class="card-body">
						...
						<table id="alliances" class="table table-striped table-bordered" style="width:100%" v-if="!allyLoading">
							...
							<tbody>
								...
								<tr>
									<td class="text-right">Real score</td>
									<td class="text-right">{{ alliance.real_score.toLocaleString() }}</td>
									<td colspan="2"></td>
								</tr>
		...
		methods: {
			loadAlliance: function() {
				...
				.then((response) => {
					...
					this.alliance.real_score = this.alliance.score - (this.alliance.hidden_resources / 20) + (this.alliance.hidden_resources / 10);
					this.alliance.real_score = Math.ceil(this.alliance.real_score);
