<template>
	<main class="my-3">
		...
		<div class="row justify-content-center">
					...
					<div class="card-body">
						<table id="alliances" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
							...
							<tbody>
								...
								<tr v-for="teamup in battlegroup.claims" >
									<td>{{ teamup.battlegroup_name }}: {{ teamup.name }}</td>
									<td class="text-right" >
										<div  v-if="booking.user_id == settings.user.id">
											<button v-on:click="removeTeamup(teamup.teamup_id)" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>	
										</div>
									</td>
								</tr>
							</tbody>
			...
			<div class="col-md-4" v-if="booking.user_id == settings.user.id">
				<div class="card card-default mb-3">
					...
					<div class="card-body">
						...
						<form @submit.prevent="addUser" v-if="nonusers.length && booking.user_id == settings.user.id">
						   <label for="addUser">Alliance</label><br /> <select v-model="user" id="addUser">
						...
						<br />
						<form @submit.prevent="addTeamup" v-if="booking.user_id == settings.user.id">
							<label for="battlegroupFleets">Battlegroup</label><br /> 
							<select name="battlegroupTeamups" id="battlegroupTeamups">
								<option v-for="(teamup,index) in battlegroup.teamups" :value="'teamup_' + teamup.teamup_id">
								  {{ teamup.name }}
								</option>
							</select>
							<button type="submit" class="btn btn-primary">Add</button>
		...
		methods: {
			...
			loadBattlegroup: function() {
				...
				.then((response) => {
					...
					axios.get('api/v1/battlegroup/users/' + this.user.id + '/booking/' + this.$route.params.booking_id)
					.then((response) => {
			...
			addTeamup: function() {
				this.battlegroupTeamups = document.getElementById('battlegroupTeamups');
				this.battlegroupTeamups = this.battlegroupTeamups.options[this.battlegroupTeamups.selectedIndex].value
				this.battlegroupTeamups = this.battlegroupTeamups.substr(this.battlegroupTeamups.indexOf('_') + 1)
				axios.get('api/v1/attacks/booking/' + this.$route.params.booking_id + '/battlegroup/' + this.battlegroup.battlegroup_id + '/teamup/' + this.battlegroupTeamups + '/add')
				.then((response) => {
					this.loadBooking();
					this.loadNonusers();
					this.loadUsers();
			...
			removeTeamup: function(teamupId) {
				axios.get('api/v1/attacks/booking/' + this.$route.params.booking_id + '/battlegroup/' + this.battlegroup.battlegroup_id + '/teamup/' + teamupId + '/remove')
				.then((response) => {
					this.loadBooking();
					this.loadNonusers();
					this.loadUsers();
