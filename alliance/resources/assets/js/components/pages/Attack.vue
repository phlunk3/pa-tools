<template>
	<main class="my-3">
		<div class="row justify-content-center attack">
							...
							<tbody>
								<tr v-for="(target, number) in attack.targets">
									...
									<td class="text-center align-middle book" v-for="booking in target.bookings">
										<span v-if="!booking.user_id && booking.user_id != user.id">
											<a v-on:click="book(number, target.id, booking.id)" class="btn-link btn-sm"><i class="fas fa-book"></i></a>
										</span>
										<span v-if="booking.user_id && booking.user_id == user.id">
											<span v-if="battlegroup.name && booking.user_id && !booking['booking_' + booking.id]" v-on:click="show(booking.id)" :id="'battlegroupTeamupsName_' + booking.id" class="claimed">
												{{ battlegroup.name }}
											</span>
											<span v-if="booking.user_id && battlegroup.bookings">										
												<form @submit.prevent="addTeamup">
													<select v-on:blur="hide(number, target.id, booking.id);" :id="'battlegroupTeamups_' + booking.id" style="display:none;">
														<option value="none"> </option>
														<option v-for="(teamup,index) in battlegroup.bookings[booking.id]" :value="'teamup_' + teamup.teamup_id">
															<span v-if="teamup.claimed">-</span>
															<span v-if="!teamup.claimed">+</span> {{ teamup.name }}
														</option>
													</select>
										...
										<span v-if="!battlegroup.name && booking.user_id && booking.user_id == user.id" class="claimed">
											{{ booking.user.name }}
										</span>
										
										<span v-if="booking.user_id && booking.user_id != user.id && booking.user">
											{{ booking.user.name }}
										</span>
			...
			<div class="col-md-6">
			  <div class="card card-default mb-3" v-if="settings.role == 'Admin' || settings.role == 'BC' || settings.role == 'Scanner'">
				  <div class="card-header">
						Targets
						<div class="float-right">
...
<style>
	.claimed {font-weight:bold;}
</style>
<script>
	export default {
		props: ['settings', 'id', 'user_id'],
		data() {
			return {
				'attack': {},
				'bookings': {},
		...
		methods: {
			...
			hide: function (number, targetId, bookingId) {
				
				//first hide the content
				document.getElementById('battlegroupTeamups_' + bookingId).style.display = 'none'
				document.getElementById('battlegroupTeamupsName_' + bookingId).style.display = 'block'
				
				// now process selection
				this.teamupId = document.getElementById('battlegroupTeamups_' + bookingId);
				this.bookingId = bookingId;
				this.teamupAction = this.teamupId.options[this.teamupId.selectedIndex].innerHTML.trim();
				this.teamupId = this.teamupId.options[this.teamupId.selectedIndex].value
				
				// check for valid selection
				if (this.teamupId != 'none') {
					
					// either add or remove teamup
					this.teamupId = this.teamupId.substr(this.teamupId.indexOf('_') + 1)
					console.log(this.teamupId);
					
					if (this.teamupAction.indexOf('<span>+</span>') != -1) {
						axios.get('api/v1/attacks/booking/' + bookingId + '/battlegroup/' + this.battlegroup.id + '/teamup/' + this.teamupId + '/add')
						.then((response) => {
							
							this.selectedOption = document.getElementById('battlegroupTeamups_' + this.bookingId);
							this.selectedOption.options[this.selectedOption.selectedIndex].innerHTML = this.selectedOption.options[this.selectedOption.selectedIndex].innerHTML.replace('+', '-');
							
							this.$notify({
								group: 'foo',
								title: 'Success',
								text: 'Battlegroup Teamup Added',
								type: 'success'
							});
						});
						
						// process displayed values
						this.teamupSelected = document.getElementById('battlegroupTeamups_' + bookingId);
						this.teamupSelected.options[this.teamupSelected.selectedIndex].innerHTML.replace('<span>+</span>', '<span>-</span>');
					} else {
						axios.get('api/v1/attacks/booking/' + bookingId + '/battlegroup/' + this.battlegroup.id + '/teamup/' + this.teamupId + '/remove')
						.then((response) => {
							this.selectedOption = document.getElementById('battlegroupTeamups_' + this.bookingId);
							this.selectedOption.options[this.selectedOption.selectedIndex].innerHTML = this.selectedOption.options[this.selectedOption.selectedIndex].innerHTML.replace(/\-/g, '+');
							
							this.$notify({
								group: 'foo',
								title: 'Success',
								text: 'Battlegroup Teamup Removed',
								type: 'success'
							});
			...
			show: function (bookingId) {
				if (document.getElementById('battlegroupTeamups_' + bookingId))
					document.getElementById('battlegroupTeamups_' + bookingId).style.display = 'block'
				if (document.getElementById('battlegroupTeamupsName_' + bookingId))
					document.getElementById('battlegroupTeamupsName_' + bookingId).style.display = 'none'
			...
			loadBattlegroup: function() {
				...
				.then((response) => {
					...
					.then((response) => {
						...
						axios.get('api/v1/battlegroup/users/' + this.user.id + '/attack/' + this.attack.id + '/bookings')
						.then((response) => {
