<template>
	<div class="card card-default mb-3" v-bind:class="{ 'has-table': pager.total }">
		<div class="card-header">
			Incoming Fleets
			<router-link :to="{ name: type + 'Incoming', params: { id: id }}" class="float-right" v-if="pager.total > pager.per_page && !loading"><button class="btn btn-sm btn-link">show all</button></router-link>
		</div>
		...
		<div class="card-body">
			...
			<table id="members" class="table table-striped table-bordered" style="width:100%" v-if="!loading && pager.total">
				<thead>
					<tr>
					  <th>Tick</th>
					  <th>From</th>
					  <th v-if="type == 'alliance' || type == 'galaxy'">Target</th>
					  <th>Fleet</th>
					  <th>Eta</th>
					  <th>Ships</th>
					  <th>Ally From</th>
				...
				<tbody>
					<tr v-for="fleet in data" v-bind:class="{ missionattack: fleet.mission_type == 'Attack', missiondefend: fleet.mission_type == 'Defend' }" v-if="fleet.planet_to && fleet.planet_from">
						...
						<td v-if="type == 'alliance' || type == 'galaxy'"><router-link :to="{ name: 'galaxy', params: { id: fleet.planet_to.galaxy_id }}"><a>{{ fleet.planet_to.x }}:{{ fleet.planet_to.y }}</a></router-link> <router-link :to="{ name: 'planet', params: { id: fleet.planet_to.id }}"><a>{{ fleet.planet_to.z }}</a></router-link></td>
			...
			<p v-if="!pager.total && !loading" class="no-data">No incoming fleets tracked</p>
		</div>
	</div>
</template>
