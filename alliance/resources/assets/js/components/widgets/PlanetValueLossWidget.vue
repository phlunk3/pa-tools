<template>
	<div class="card card-default has-table">
		<div class="card-header">Biggest value loss</div>
			...
			<table id="members" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
				...
				<tbody>
					<tr v-for="(planet, index) in pager.data" v-bind:class="{ is_alliance: planet.is_alliance }">
						...
						<td class="text-right">{{ planet.value | shorten }} (<span v-bind:class="{ yellow: planet.growth_value == 0, green: planet.growth_value > 0, red: planet.growth_value < 0 }">{{ planet.growth_value | shorten }}</span>)</td>
						<td class="text-right" style="width: 100%">{{ planet.nick }} <span v-if="planet.nick && planet.alliance"> </span><router-link v-if="planet.alliance" :to="{ name: 'alliance', params: { id: planet.alliance.id }}"><a>{{ planet.alliance.name }}</a></router-link></td>
					</tr>
				</tbody>
			</table>
		</div>
