			...
			<table id="members" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
			  ...
			  <tbody>
				  <tr v-for="(alliance, index) in pager.data" v-bind:class="{ is_alliance: alliance.is_alliance }">
					  ...
					  <td class="text-right">{{ alliance.value | shorten }} (<span v-bind:class="{ yellow: alliance.growth_value == 0, green: alliance.growth_value > 0, red: alliance.growth_value < 0 }">{{ alliance.growth_value | shorten }}</span>)</td>
				  </tr>
			  </tbody>
			</table>
		</div>
