<template>
	<div class="card card-default has-table">
		<div class="card-header">Biggest score gains</div>
			...
			<table id="members" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
			  ...
			  <tbody>
				  <tr v-for="(alliance, index) in pager.data" v-bind:class="{ is_alliance: alliance.is_alliance }">
					  ...
					  <td class="text-right">{{ alliance.score | shorten }} (<span v-bind:class="{ yellow: alliance.growth_score == 0, green: alliance.growth_score > 0, red: alliance.growth_score < 0 }">{{ alliance.growth_score | shorten }}</span>)</td>
				  </tr>
			  </tbody>
			</table>
		</div>
