			...
			<table id="members" class="table table-striped table-bordered" style="width:100%" v-if="!loading">
				...
				<tbody>
					<tr v-for="(planet, index) in pager.data" v-bind:class="{ is_alliance: planet.is_alliance }">
						...
						<td class="text-right">{{ planet.size | shorten }} (<span v-bind:class="{ yellow: planet.growth_size == 0, green: planet.growth_size > 0, red: planet.growth_size < 0 }">{{ planet.growth_size | shorten }}</span>)</td>
